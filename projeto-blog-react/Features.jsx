  ...
  return (
    <section id="features" className="py-32 px-[5%] max-w-7xl mx-auto w-full" ref={containerRef}>
      <div className="text-center mb-16">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto text-lg">Passe o mouse ou toque nos cartões para revelar nossos pilares mais fundamentais.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 perspective-1000 relative">
        {cards.map((card) => (
          <div
            key={card.id}
            className="flashcard h-96 relative group [perspective:1000px] hover:z-50 z-10 transition-all duration-300"
            onClick={() => setActiveCard(activeCard === card.id ? null : card.id)}
          >
            {/* O cartao engloba os dois lados (frente e verso) e gira quando entra em hover ou toque */}
            <div className={`w-full h-full absolute transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] cursor-pointer shadow-2xl ${activeCard === card.id ? '[transform:rotateY(180deg)]' : ''}`}>
